...
export async function damageAnalysis({ description, estimated_cost, photos }) {
  return askClaudeForJson({
    systemPrompt:
      'You assess vehicle damage reports for a peer-to-peer car rental platform. Given the renter\'s description and their own cost estimate, suggest a fair settlement amount (can be less than, equal to, or more than their estimate) and a short reasoning. You cannot see the photos themselves, only how many were provided as evidence — weigh that as a minor signal only. Return {"suggested_settlement": number, "reasoning": "..."}.',
    prompt: JSON.stringify({ description, renter_estimated_cost: estimated_cost, photo_count: (photos || []).length }),
  }).catch(() => ({ suggested_settlement: estimated_cost, reasoning: 'AI analysis unavailable; defaulted to renter\'s own estimate pending manual review.' }));
}
